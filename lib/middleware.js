...
module.exports = less.middleware = function(source, options, parserOptions, compilerOptions){
  ...
  // allows response to be sent after file is written
  var storeCssCallback = (function(sendResponse) {
    if (sendResponse) {
      return function(css, res, _) {
        return function() {
          res.type('css');
          res.send(css);
        };
  ...
  })((options && options.sendResponse) || false);
  ...
  options = extend(true, {
    ...
    storeCss: function(pathname, css, next, res) {
      mkdirp(path.dirname(pathname), 511 /* 0777 */, function(err){
        if (err) return next(err);

        fs.writeFile(pathname, css, 'utf8', storeCssCallback(css, res, next));
      });
    }
  }, options || {});
        ...
        fs.readFile(lessPath, 'utf8', function(err, lessSrc){
          ...
          try {
            ...
            render(lessSrc, lessPath, cssPath, function(err, css){
              ...
              options.storeCss(cssPath, css, next, res);
            });
          } catch (err) {
            utilities.lessError(err);
            return next(err);
